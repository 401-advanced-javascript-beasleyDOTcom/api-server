...
function getAllProducts (request, response, next){
    console.log('you have reached getAllProducts', products)
    products.get()
        .then(results => {
            console.log('this is inside of the getAllProducts', results)
            let count = results.length;
            response.status(200).json({'count': count, 'results':results});
            next();
        }).catch((err)=> console.log(err,'this is inside of the catch in getAllProducts'));
};
